...
import com.hrm.test.api.data.model.common.GsonToModelResponse;
...
public class GsonFunctions {
    ...
    // Method used to parse successful Json response as model class
    public static <T> T parseSuccessResponseToModel(String json, Class<T> classOfT){
        GsonToModelResponse<T> model = new GsonToModelResponse<>(new GsonBuilder().create().fromJson(json, (Type) classOfT));
        if (((ErrorResponse) model.getData()).getValue() == null) {
            return (T) model.getData();
        } else {
            Assert.fail("Endpoint for processing class " + classOfT + " return error: " + ((ErrorResponse) model.getData()).getValue());
            return null;
        }
    }

    // Method used to parse error Json response as model class
    public static <T> T parseErrorResponseToModel(String json, Class<T> classOfT){
        ...
        if (((ErrorResponse) model.getData()).getValue() != null) {
            ...
            Assert.fail("Endpoint for processing class " + classOfT + " didn't return error: " + "data[" +
                    (T)  model.getData().toString() + "]");
            return null;
        }
    }

    public static <T> T parseAnyResponseToModel(String json, Class<T> classOfT){
        GsonToModelResponse<T> model = new GsonToModelResponse<>(new GsonBuilder().create().fromJson(json, (Type) classOfT));
        return (T) model.getData();
    }
}
